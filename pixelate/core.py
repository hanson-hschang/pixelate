...
class PixelArtParser:
    ...
    def parse_markdown_file(self, file_path: Path) -> Tuple[Dict[str, str], List[List[str]]]:
        """
...
class ImageGenerator:
    ...
    def generate_pixel_image(self, color_dict: Dict[str, str], pixel_grid: List[List[str]], 
...
class FileProcessor:
    ...
    def process_markdown_file(self, markdown_file: Path, output_dir: Optional[Path] = None, 
                             pixel_size: int = 10, format: str = 'png') -> None:
        """Process a single markdown file and generate its image."""
        try:
            print(f"Processing file: {markdown_file}")
            
            # Parse the markdown file
            color_dict, pixel_grid = self.parser.parse_markdown_file(markdown_file)
            ...
            self.generator.generate_pixel_image(color_dict, pixel_grid, output_path, pixel_size, format)
            
        except Exception as e:
            print(f"Error processing {markdown_file}: {e}")
    
    def process_folder(self, folder_path: Path, pixel_size: int = 10, format: str = 'png') -> None:
        """Process all markdown files in a folder."""
        # Find all markdown files
        markdown_files = self.find_markdown_files(folder_path)
    ...
    def run(self, input_path: str, pixel_size: int = 10, format: str = 'png') -> None:
        ...
        if path.is_file():
            if path.suffix.lower() != '.md':
                ...
                sys.exit(1)
            self.processor.process_markdown_file(path, None, pixel_size, format)
        elif path.is_dir():
            self.processor.process_folder(path, pixel_size, format)
        else:
